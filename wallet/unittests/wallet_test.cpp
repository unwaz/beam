...
namespace
{
    ...
    public:
        ...
        void GenerateOutputs(IWalletDB& db, const AmountList& amounts, Height minHeight)
        {
            for (const auto& amount : amounts)
            {
                m_Outputs.push_back(CreateOutput(db, amount, minHeight));
            }
        }
        ...
        std::vector<Cookie> GenerateCookies(IWalletDB& db, const AmountList& amounts, Height minHeight, Height maxHeight)
        {
            ...
            for (const auto& amount : amounts)
            {
                // create output
                Coin newUtxo(amount);
                db.storeCoin(newUtxo);

                Scalar::Native blindingFactor;
                Output::Ptr output = make_unique<Output>();
                output->Create(minHeight, blindingFactor, *db.get_ChildKdf(newUtxo.m_ID), newUtxo.m_ID, *db.get_MasterKdf());
        ...
        Output::Ptr CreateOutput(IWalletDB& db, Amount amount, Height minHeight)
        {
            Coin newUtxo(amount);
            db.storeCoin(newUtxo);
    ...
    BaseTransaction::Ptr CreateOneSideTransaction(TestWalletRig& rig, const WalletID& from, const WalletID& to, Amount amount, Amount fee)
    {
        TxID txID = GenerateTxID();
        auto tx = OneSideTransaction::Create(rig.m_Wallet, rig.m_WalletDB, rig.m_KeyKeeper, txID);

        tx->SetParameter(TxParameterID::TransactionType, TxType::OneSide, false);
        tx->SetParameter(TxParameterID::Lifetime, kDefaultTxLifetime, false);
        tx->SetParameter(TxParameterID::PeerResponseHeight, kDefaultTxResponseTime);
        tx->SetParameter(TxParameterID::IsInitiator, true, false);
        tx->SetParameter(TxParameterID::AmountList, AmountList{amount}, false);
      //  tx->SetParameter(TxParameterID::PreselectedCoins, {}, false);
        ...
        txDescription.m_status = TxStatus::Pending;
        txDescription.m_selfTx = false;
        rig.m_WalletDB->saveTx(txDescription);
        return tx;
    }
    ...
    void TestOneSideTransaction()
    {
        ...
        OneSideReceiver oneSideReceiver;
        {
            ...
            auto tx = CreateOneSideTransaction(sender, sender.m_WalletID, receiver.m_WalletID, 4, 1);
        ...
        }
        {
            ...
            auto tx = CreateOneSideTransaction(sender, sender.m_WalletID, receiver.m_WalletID, 5, 0);
